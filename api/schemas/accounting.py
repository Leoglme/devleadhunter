...
class StripePaymentInfo(BaseModel):
    ...
    session_id: str | None = Field(None, description="Stripe Checkout Session ID")
    ...
    payment_method_type: str | None = Field(None, description="Payment method type (card, etc.)")
    payment_method_brand: str | None = Field(None, description="Payment method brand (Visa, Mastercard, etc.)")
    payment_method_last4: str | None = Field(None, description="Last 4 digits of the payment method")
    payment_date: datetime = Field(..., description="Payment date")
    amount_received: Decimal | None = Field(None, description="Amount received after fees")
    application_fee_amount: Decimal | None = Field(None, description="Stripe application fee")
    net_amount: Decimal | None = Field(None, description="Net amount after all fees")
    available_at: datetime | None = Field(None, description="When funds become available (if provided by Stripe)")
    refund_amount: Decimal | None = Field(None, description="Refunded amount if any")
    refund_date: datetime | None = Field(None, description="Refund date if any")
    customer_country: str | None = Field(None, description="Customer country code")
    customer_name: str | None = Field(None, description="Customer name")
    customer_email: str | None = Field(None, description="Customer email")
    ip_address: str | None = Field(None, description="Customer IP address")
...
class CreditPurchaseTransaction(BaseModel):
    ...
    payment_info: StripePaymentInfo | None = Field(None, description="Stripe payment details")
    euros_amount: Decimal | None = Field(None, description="Amount paid in euros")
...
class AccountingSummary(BaseModel):
    ...
    available_balance: Decimal | None = Field(None, description="Available balance on Stripe account")
