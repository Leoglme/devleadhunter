...
class CreditTransactionBase(BaseModel):
    ...
    transaction_type: str = Field(..., description="Transaction type: PURCHASE, USAGE, REFUND, FREE_GIFT")
    amount: int = Field(..., description="Number of credits (positive for additions, negative for usage)")
    description: str = Field(
        ..., min_length=1, max_length=500, description="Human-readable description of the transaction"
    )
    transaction_metadata: str | None = Field(
        None, max_length=1000, description="Optional JSON metadata for additional transaction information"
    )
