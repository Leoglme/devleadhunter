...
class StripePaymentService:
    ...
    def create_checkout_session(
        self, db: Session, user_id: int, credits: int, success_url: str, cancel_url: str
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            checkout_session = self.stripe_client.checkout.Session.create(
                payment_method_types=["card"],  # Cartes + Apple/Google Pay (activés automatiquement)
                line_items=[
                    {
                        "price_data": {
                            "currency": "eur",
            ...
            return {"id": checkout_session.id, "url": checkout_session.url, "amount": amount_cents, "credits": credits}

        except stripe.error.StripeError as e:
            raise Exception(f"Stripe error: {e!s}")
    ...
    def create_order_payment_link(
        ...
        *,
        ...
        try:
            ...
            raise Exception(f"Stripe error: {exc!s}")
        ...
        # Handle payment intent succeeded (alternative webhook)
            ...
            if user_id > 0 and credits > 0:
                ...
                for transaction in existing_transactions:
                    if (
                        transaction.transaction_metadata
                        and f"stripe_payment_intent:{payment_intent_id}" in transaction.transaction_metadata
                    ):
                        # Already processed
                        return True
    ...
    def verify_webhook_signature(self, payload: bytes, signature: str) -> dict[str, Any] | None:
        ...
        try:
            event = self.stripe_client.Webhook.construct_event(payload, signature, settings.stripe_webhook_secret)
            return event
...
stripe_payment_service: StripePaymentService | None = None


def get_stripe_service() -> StripePaymentService | None:
    """
