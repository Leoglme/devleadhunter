...
Best-effort by design: ``notify`` never raises, so a push failure can never break
the request (email webhook, demo event, sale) that triggered it. Stale endpoints
...
def send_push(
    ...
    Args:
        ...
        tag: Optional notification tag (collapses/replaces same-tag notifications).
    ...
    payload_data: dict[str, str] = {"title": title, "body": body, "url": url}
    if tag:
    ...
    for subscription in subscriptions:
        try:
            ...
            result.failed += 1
            result.details.append(f"HTTP {status_code}: {str(exc)[:160]}")
            logger.warning("Push failed (user=%s status=%s): %s", user_id, status_code, exc)
...
async def notify(user_id: int, title: str, body: str, url: str = "/dashboard", tag: str | None = None) -> None:
    """
    ...
    def _run() -> None:
        db = SessionLocal()
        try:
            send_push(db, user_id, title, body, url, tag)
        finally:
            db.close()
