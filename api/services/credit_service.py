...
class CreditService:
    ...
    def get_user_credits_consumed(db: Session, user_id: int) -> int:
        ...
        result = db.execute(
            select(func.sum(CreditTransaction.amount)).where(
                CreditTransaction.user_id == user_id, CreditTransaction.amount < 0
            )
        ).scalar()
    ...
    def create_transaction(
        db: Session, user_id: int, transaction_type: str, amount: int, description: str, metadata: str | None = None
    ) -> CreditTransaction:
        """
    ...
    @staticmethod
    def use_credits(db: Session, user_id: int, amount: int, description: str, metadata: str | None = None) -> bool:
        """
    ...
    @staticmethod
    def get_user_transactions(db: Session, user_id: int, limit: int = 100, skip: int = 0) -> list[CreditTransaction]:
        """
