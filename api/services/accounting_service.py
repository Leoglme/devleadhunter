...
from schemas.accounting import AccountingSummary, CreditPurchaseTransaction, StripePaymentInfo
...
class AccountingService:
    ...
    def get_stripe_balance(self) -> Decimal | None:
        """
    ...
    def get_payment_intent_details(self, payment_intent_id: str) -> dict[str, Any] | None:
        """
        ...
        try:
            payment_intent = self.stripe_client.PaymentIntent.retrieve(
                payment_intent_id,
                expand=["charges.data.balance_transaction", "charges.data.payment_method", "payment_method", "refunds"],
            )
    ...
    def get_checkout_session_details(self, session_id: str) -> dict[str, Any] | None:
        """
    ...
    def find_stripe_session_by_email_and_date(
        self, email: str, transaction_date: datetime, time_window_hours: int = 24
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            sessions = self.stripe_client.checkout.Session.list(
                limit=100, created={"gte": start_time}, expand=["data.payment_intent", "data.payment_intent.charges"]
            )
    ...
    def extract_stripe_payment_info(
        self, transaction_metadata: str | None, created_at: datetime, user_email: str | None = None
    ) -> StripePaymentInfo | None:
        """
        ...
        try:
            ...
            if session_id:
                ...
                if session:
                                ...
                                try:
                                    if isinstance(latest_charge_id, str):
                                        charge_detail = self.stripe_client.Charge.retrieve(
                                            latest_charge_id, expand=["balance_transaction"]
                                        )
                                    else:
                    ...
                    payment_info = StripePaymentInfo(
                        ...
                        application_fee_amount=Decimal(application_fee_amount) / 100
            ...
            if not payment_info and user_email:
                session = self.find_stripe_session_by_email_and_date(
                    email=user_email, transaction_date=created_at, time_window_hours=48
                )
    ...
    def get_all_stripe_charges(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        try:
            charges = self.stripe_client.Charge.list(
                limit=limit, expand=["data.balance_transaction", "data.payment_intent", "data.customer"]
            )
            ...
            for charge in charges.data:
                ...
                net_amount = (
                    (amount_received - application_fee_amount) / 100
                    if amount_received and application_fee_amount
                    else None
                )
                ...
                payment_info = StripePaymentInfo(
                    ...
                    payment_method_brand=charge.get("payment_method_details", {}).get("card", {}).get("brand")
                    if charge.get("payment_method_details") and charge.get("payment_method_details", {}).get("card")
                    else None,
                    payment_method_last4=charge.get("payment_method_details", {}).get("card", {}).get("last4")
                    if charge.get("payment_method_details") and charge.get("payment_method_details", {}).get("card")
                    else None,
                    ...
                    customer_country=charge.get("billing_details", {}).get("address", {}).get("country")
                    ...
                    refund_date=datetime.fromtimestamp(charge.get("refunded", {}).get("created", 0))
                ...
                result.append(
                    {"charge_id": charge.get("id"), "payment_info": payment_info, "created": charge.get("created", 0)}
                )
    ...
    def get_stripe_payment_intents(self, limit: int = 100, statuses: list[str] | None = None) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            for intent in payment_intents.data:
                ...
                amount_received_decimal = (
                    Decimal(amount_received_value) / Decimal("100") if amount_received_value is not None else None
                )
                ...
                customer_country = None
                customer_name = None
                application_fee_decimal: Decimal | None = Decimal("0")
                available_on_timestamp: int | None = None
                refund_amount_decimal: Decimal | None = None
                ...
                if not charges:
                    try:
                        charge_list = self.stripe_client.Charge.list(
                            payment_intent=intent_id, limit=1, expand=["data.balance_transaction"]
                        )
                ...
                available_on_datetime = (
                    datetime.fromtimestamp(available_on_timestamp) if available_on_timestamp else None
                )

                net_amount_decimal: Decimal | None = None
                ...
                results[intent_id] = {"payment_info": payment_info, "created": created_ts, "metadata": metadata}
    ...
    def get_stripe_checkout_sessions(self, limit: int = 100, statuses: list[str] | None = None) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            sessions = self.stripe_client.checkout.Session.list(
                limit=fetch_limit, expand=["data.payment_intent", "data.customer"]
            )
            ...
            for session in sessions.data:
                ...
                results.append({"session": session, "metadata": session.get("metadata") or {}})
    ...
    def get_accounting_data(self, db: Session, skip: int = 0, limit: int = 100) -> dict[str, Any]:
        """
        ...
        session_statuses = ["open", "expired", "complete"]
        ...
        for session_data in stripe_sessions:
            ...
            stripe_transactions.append({"payment_info": payment_info, "created": created_ts, "metadata": metadata})
        ...
        for transaction_data in stripe_transactions:
            ...
            resolved_customer_name = (
                db_user.name
                if db_user
                else payment_info.customer_name
                or metadata_customer_name
                or payment_info.customer_email
                or "Client Stripe"
            )
            resolved_customer_email = (
                db_user.email if db_user else payment_info.customer_email or metadata_customer_email or "Unknown"
            )
            ...
            updated_payment_info = payment_info.copy(
                update={
        ...
        return {"summary": summary, "transactions": paginated_transactions}
