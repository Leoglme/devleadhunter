...
async def get_my_balance(
    ...
    return CreditBalanceResponse(
        user_id=current_user.id, balance=balance if not is_unlimited else 0, is_unlimited=is_unlimited
    )
...
async def get_user_balance(
    ...
    return CreditBalanceResponse(user_id=user_id, balance=balance if not is_unlimited else 0, is_unlimited=is_unlimited)


@router.get("/transactions", response_model=list[CreditTransactionResponse])
async def get_my_transactions(
    skip: int = 0, limit: int = 100, current_user: User = Depends(require_auth), db: Session = Depends(get_db)
    ...
    """
    transactions = credit_service.get_user_transactions(db, current_user.id, limit=limit, skip=skip)
    return transactions
...
async def add_credits(
    ...
    transaction = credit_service.add_credits(
        db=db, user_id=user_id, amount=amount, description=description, transaction_type=TransactionType.PURCHASE
    )
...
async def use_credits(
    ...
    success = credit_service.use_credits(
        db=db, user_id=current_user.id, amount=amount, description=description, metadata=metadata
    )
